...
session_start();
include 'db.php';
include 'includes/fonctions/notifications.php'; // Inclure la fonction de notification
...
if (isset($_GET['id'])) {
    ...
    if (mysqli_num_rows($check) === 0) {
        ...
        // ➕ Ajouter une notification si l'article est en achat immédiat
        $q = mysqli_query($db, "SELECT type_vente, vendeur_id FROM articles WHERE id = $article_id");
        if ($data = mysqli_fetch_assoc($q)) {
            if (strtolower($data['type_vente']) === 'immediate') {
                $vendeur_id = intval($data['vendeur_id']);
                envoyer_notification($db, $vendeur_id, "Un client a ajouté votre article #$article_id à son panier (achat immédiat).");
            }
        }
    }
